...
const NumberButtonBox: React.FC<BoxProps> = (boxProps) => {
    ...
    return (
        <div className={`w-full [background:var(--wt-100,#FFF)] shadow-[0_-8px_30px_rgba(0,0,0,0.25)] sm:shadow-[0px_10px_20px_0px_rgba(0,0,0,0.10),0px_36px_72px_0px_rgba(0,0,0,0.10)] rounded-t-[48px] sm:rounded-[48px] overflow-hidden ${boxProps.className}`}>
            ...
            <div className="flex items-center flex-col sm:items-start sm:flex-row sm:min-h-[400px]">
                <div className="w-full sm:w-1/2 min-w-[400px] sm:min-w-[50%] px-[48px] pt-8 sm:pt-[56px] pb-8 sm:pb-[48px]">
                    <h3 className="text-[color:var(--GY-700,#555)] [font-family:'Spoqa_Han_Sans_Neo'] text-base sm:text-xl font-medium leading-7 mb-2 sm:mb-4">입력하신 전화번호로 <br className="hidden sm:block" />체험 메시지가 발송됩니다.</h3>
                    <p className=" text-[color:var(--GY-500,#808080)] [font-family:'Spoqa_Han_Sans_Neo'] text-xs font-normal leading-4">입력된 휴대전화번호는 메시지 발송 이외에 <br className="hidden sm:block" />다른 목적으로 사용하거나 수집하지 않습니다.</p>
                </div>
                <div className="buttons-wrap w-full sm:w-1/2 min-w-[400px] sm:min-w-[50%] px-[70px] pt-6 sm:pt-[40px] pb-[72px] flex flex-wrap justify-between">
                    { NumberButtonsRender() }
                </div>
            </div>
        </div>
