    ...
    return (
        <div className={`w-full [background:var(--wt-100,#FFF)] shadow-[0_-8px_30px_rgba(0,0,0,0.25)] sm:shadow-[0px_10px_20px_0px_rgba(0,0,0,0.10),0px_36px_72px_0px_rgba(0,0,0,0.10)] rounded-t-[48px] sm:rounded-[48px] overflow-hidden ${boxProps.className}`}>
            <div className="[background:var(--GY-20,#FAFAFA)] flex flex-col items-start gap-2 flex-[1_0_0] p-12">
                <p className="text-[color:var(--GY-900,#2B2B2B)] text-xl font-bold leading-6 mb-2">전화번호를 입력해 주세요.</p>
                <div className="flex items-center text-[40px] font-bold leading-[normal]">
            ...
            <div className="flex items-center flex-col sm:items-start sm:flex-row sm:min-h-[400px]">
                <div className="w-full sm:w-1/2 min-w-[400px] sm:min-w-[50%] px-[48px] pt-8 sm:pt-[56px] pb-4 sm:pb-[48px]">
                    ...
                    <p className=" text-[color:var(--GY-500,#808080)] text-xs font-normal leading-4">입력된 휴대전화번호는 메시지 발송 이외에 <br className="hidden sm:block" />다른 목적으로 사용하거나 수집하지 않습니다.</p>
                </div>
                <div className="buttons-wrap w-full sm:w-1/2 min-w-[400px] sm:min-w-[50%]  px-[32px] sm:px-[70px] pt-6 sm:pt-[40px] pb-[40px] sm:pb-[72px] flex flex-wrap justify-between">
                    { NumberButtonsRender() }
                </div>
            </div>
        </div>
